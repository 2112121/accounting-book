...
const RecurringIncomeManagement: React.FC<RecurringIncomeManagementProps> = ({
  ...
  const handleUpdateRule = async (e: React.FormEvent, ruleId: string) => {
    ...
    try {
      ...
      await updateDoc(doc(db, "recurringIncomes", ruleId), {
      ...
      setSuccess("定期收入規則已更新");
      cancelInlineEdit();
      await loadRules();
    } catch (_error) {
      setError("更新定期收入規則失敗，請稍後再試");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleCreateRule = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await addDoc(collection(db, "recurringIncomes"), {
      ...
      setSuccess("定期收入規則已建立");
      resetForm();
      await loadRules();
    } catch (_error) {
      setError("建立定期收入規則失敗，請稍後再試");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDeleteRule = async (ruleId: string) => {
    if (!window.confirm("確定要停止這條定期收入嗎？這不會刪掉已產生的收入記錄。")) {
      return;
    }
    ...
    try {
      ...
      setSuccess("定期收入規則已停止");
    } catch (_error) {
      setError("停止定期收入規則失敗，請稍後再試");
    }
  };
    ...
    <div className="p-5">
      ...
      <div className="mb-5 rounded-xl border border-[#CDE8F8] bg-[#F2FAFF] p-4">
        <p className="text-sm text-gray-700 leading-6">
          每月與每年規則會以開始日期作為基準日。若該月份沒有對應日期（例如 31 號遇到二月），系統會自動調整至該月最後一天，並在下個有該日期的月份恢復原始日期。修改金額僅影響未來自動產生的收入，已產生的過去記錄不受影響。
        </p>
      </div>
      ...
      <form onSubmit={handleCreateRule} className="space-y-4 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
        <div className="grid grid-cols-2 gap-3">
          <div className="col-span-2">
            ...
            <input
              ...
              className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-[#4EA8DE] focus:outline-none focus:ring-2 focus:ring-[#D6ECF9]"
              ...
              {(["daily", "weekly", "monthly", "yearly"] as RecurringPeriod[]).map((item) => (
                <button
                  ...
                  className={`rounded-lg border px-3 py-2 text-sm font-medium transition-colors ${
                    period === item
                      ? "border-[#4EA8DE] bg-[#4EA8DE] text-white"
        ...
        <button
          type="submit"
          disabled={saving}
          className="w-full rounded-xl bg-[#4EA8DE] px-4 py-3 text-sm font-medium text-white transition-colors hover:bg-[#3D97CD] disabled:cursor-not-allowed disabled:opacity-70"
        >
          {saving ? "建立中..." : "新增定期收入規則"}
        </button>
      </form>
      ...
      <div className="mt-6">
        <div className="flex items-center justify-between mb-3">
          ...
          <button
            type="button"
            onClick={loadRules}
            className="relative flex h-9 w-9 items-center justify-center rounded-full border border-[#B8E3FF] bg-white text-transparent transition-colors hover:bg-[#EBF6FF] after:absolute after:inset-0 after:flex after:items-center after:justify-center after:text-base after:text-[#4EA8DE] after:content-['↻'] hover:after:text-[#3D97CD]"
            aria-label="重新整理定期規則"
            title="重新整理"
          >
            重新整理
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {rules.map((rule) => (
              <div key={rule.id} className={`rounded-2xl border bg-white shadow-sm transition-all ${editingRuleId === rule.id ? "border-[#4EA8DE]" : "border-gray-100"}`}>
                {/* 顯示模式 */}
                <div className="flex items-start justify-between gap-3 p-4">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="rounded-full bg-[#E6F4FC] px-2.5 py-1 text-xs font-medium text-[#4EA8DE]">
                        {periodLabels[rule.period]}
                      </span>
                      <span className="text-sm font-semibold text-gray-800">{rule.category}</span>
                      <span className="text-sm text-[#4EA8DE] font-semibold">NT$ {rule.amount}</span>
                    </div>
                  ...
                  <div className="flex flex-col gap-1.5 shrink-0">
                    <button
                      ...
                      className={`rounded-lg px-3 py-2 text-sm font-medium transition-colors ${editingRuleId === rule.id ? "bg-gray-100 text-gray-500 hover:bg-gray-200" : "bg-[#E6F4FC] text-[#4EA8DE] hover:bg-[#D6ECF9]"}`}
                    >
                ...
                {editingRuleId === rule.id && editValues && (
                  <form
                    ...
                    className="border-t border-[#D6ECF9] px-4 pb-4 pt-3 space-y-3"
                  >
                    <div className="grid grid-cols-2 gap-3">
                      <div className="col-span-2">
                        <label className="block text-xs font-medium text-gray-600 mb-1">金額（加薪/降薪改這裡）</label>
                        <input
                        ...
                        <div className="grid grid-cols-4 gap-1.5">
                          {(["daily", "weekly", "monthly", "yearly"] as RecurringPeriod[]).map((item) => (
                            <button
                              ...
                              className={`rounded-lg border px-2 py-2 text-xs font-medium transition-colors ${editValues.period === item ? "border-[#4EA8DE] bg-[#4EA8DE] text-white" : "border-gray-200 bg-white text-gray-600 hover:bg-gray-50"}`}
                            >
                              {periodLabels[item]}
                            </button>
                          ))}
                      ...
                      <div>
                        <label className="block text-xs font-medium text-gray-600 mb-1">結束日期（離職可設定）</label>
                        <input
                          type="date"
                    ...
                    <button
                      type="submit"
                      disabled={saving}
                      className="w-full rounded-xl bg-[#4EA8DE] px-4 py-2.5 text-sm font-medium text-white transition-colors hover:bg-[#3D97CD] disabled:opacity-70"
                    >
                      {saving ? "儲存中..." : "儲存變更"}
                    </button>
                  </form>
...
export default RecurringIncomeManagement;
